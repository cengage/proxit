...
module.exports = {
    ...
    syncSearchOnUrlObject: function(urlObj) {
        if (urlObj.path && urlObj.path.indexOf('?') >= 0) {
            urlObj.path = urlObj.path.substr(0, urlObj.path.indexOf('?'));
        }
        urlObj.path = urlObj.path + searchString(urlObj);

        urlObj.href = urlObj.protocol + '//' + authString(urlObj) + urlObj.host + urlObj.path + hashString(urlObj);
    }
};
