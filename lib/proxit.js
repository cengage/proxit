...
function createPaths() {
    ...
    for (var i = 0; i < keys.length; i++) {
        var endpoint = routes[keys[i]],
            functions = [],
            interceptor = config.intercept;

        if (typeof endpoint === 'object') {
            interceptor = endpoint.intercept || interceptor;
            endpoint = endpoint.url;
        }

        if (typeof endpoint === 'string') {
            if (URL.parse(endpoint).host) {
                functions.push(createProxyHandler(keys[i], endpoint, interceptor));
            } else {
...
function createProxyHandler(route, url, interceptor) {
    return proxyMiddleware(route, url, interceptor);
}
