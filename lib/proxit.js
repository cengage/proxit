...
function createPaths() {
    var routes = config.routes,
        ...
        paths = [],
        proxyMiddleware = require('./proxy/proxy-middleware');
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (typeof endpoints === 'string') {
            ...
            if (options.host) {
                functions.push((function(options) {
                    ...
                    return function(req, res, next) {
                        ...
                        res.end = function() {
                            log.verbose.http(res.statusCode, path.normalize(options.protocol + '//' + options.host + req.url) + ' (' + req.url + ')');
                ...
                functions.push((function() {
                    var dir = path.resolve(path.normalize(endpoints)),
                        route = keys[i],
                        staticHandler = connect.static(dir);
